...
class BookAppAgatha extends Component{
    state={
        data:{},
        startIndex:0,
        maxResult:0,
    ...
    async fetchData(){
        ...
        let response= await http.get(`?q=Agatha Christie&${searchStr}`);
    ...
    render(){
        ...
        let {startIndex,maxResult,endIndex}=this.state;
        let {handleAddBook,booksAdded,handleRemoveBook,myBooks,selectedOptions,seloptions}=this.props;
        let queryParams=queryString.parse(this.props.location.search);
        let searchString=this.makeSearchString(queryParams);
        let size=+(seloptions.noOfpage);
        endIndex=items.length>startIndex+size-1? startIndex+size-1:
        items.length-1;
      ...
      <div className="row">
        ...
        <div className="col-9">
            <h3 className="text-warning text-center">Agatha Christie Books</h3>
