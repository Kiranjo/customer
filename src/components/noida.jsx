...
class Noida extends Component{
    handleOptionChange=(options)=>{
      
        this.callURL("/Noida/1",options);
    ...
    render(){
        ...
        let emp=employees.filter((emp)=>emp.location=="Noida");
        console.log(emp);
        let page=1;
 ...
 <div className="container">
  ...
  <div className="row">
   ...
   <div className="row">
    ...
    <Link to={`/Noida/${pageNum-1}?${searchString}`}>Prev</Link>):""}</div>

    <div className="col-8"></div>
    <div className="col-2">{endIndex < employees1.length-1 ? (
    <Link to={`/Noida/${pageNum+1}?${searchString} `}>Next</Link>):""}</div>
    </div>
  </div>
