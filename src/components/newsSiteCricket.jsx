...
class NewsSiteCricket extends Component{
    state ={data:{},
  search:{},
};
...
async fetchData(){
  ...
  // let {page="1"}=queryParams;
   let response= await http.get(`?&api-key=3b4d4934-082c-4288-b1d1-67707f4fd8b8&q=cricket?${searchStr}`);
...
handlePage=(incr)=>{
    ...
    this.callURL("&q",queryParams);
}
handleOptionChange=(options)=>{
    console.log(options);
    options.page="1";
    this.callURL("&q",options);
}
...
makeSearchString=(options)=>{
 ...
 `${paramName}=${paramValue}`:str;
  ...
  console.log(results);
  let results1=results.filter((res)=>res.sectionName=="Sport");
...
<NewsSiteLeftPanel options={queryParams}  onOptionChange={this.handleOptionChange}/>
            ...
            <div className="col-9">
            <h4>{queryParams.page>1?queryParams.page:1}-{pageSize} of {pages}</h4>
        <div className="row">
...
export default NewsSiteCricket;